...
for parent, dirnames, filenames in os.walk(rootdir):
    for filename in filenames:  # 输出文件信息
        ...
        try:
            sql = """SELECT * FROM `picindex` WHERE `filename` LIKE '%{0}%' AND `state` LIKE 'notfound'""".format(filename)
            # 执行sql语句
            cursor.execute(sql)
            if(cursor._rows == ()):
                print(filename + " skip")
                ...
                sql = """DELETE FROM `picindex` WHERE `picindex`.`id` = {0}""".format(cursor._rows[0]['id'])
                cursor.execute(sql)
